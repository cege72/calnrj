// App: CSV preview, live overrides (unit/step), PV+Export, pie with %, KPIs/economies updated by year filter,
// + FIXES: tariff edits trigger recompute; metadata parsing fixed; default unit=W when missing.
...
$('#btn-add-hc')?.addEventListener('click',()=>{ addHcRow(); });
...
// Reactivity: when tariffs/abos/ref_time/HC windows change, recompute (preserve current year)
['prix_base','prix_hc','prix_hp','abo_base','abo_hphc','ref_time'].forEach(id=>{
  $('#'+id)?.addEventListener('input', ()=> recompute(true));
  $('#'+id)?.addEventListener('change', ()=> recompute(true));
});
// When HC ranges are edited
...
function parseCSVWithMeta(text){
  ...
  // Try to detect a metadata name row followed by a value row (ENEDIS export)
  for(let i=0;i<Math.min(80,lines.length-1);i++){
    const d=detectDelimiter(lines[i]); const d2=detectDelimiter(lines[i+1]); const parts=splitCSVLine(lines[i],d).map(normalizeHeader); const partsVals=splitCSVLine(lines[i+1],d2);
    if(parts.includes('unite') && parts.includes('pas en minutes')){
      const idxU = parts.indexOf('unite'); const idxP = parts.indexOf('pas en minutes');
      unit = (partsVals[idxU]||'').trim(); const pm = (partsVals[idxP]||'').trim(); if(pm && /^\d+$/.test(pm)) pasMin = parseInt(pm,10);
    }
  ...
  if(unit==null || (pasMin==null && lines.length>0)){
    for(let i=0;i<Math.min(80,lines.length);i++){
      if(unit==null){ const m=lines[i].match(/;Unite;([^;\n]*)/i); if(m) unit=(m[1]||'').trim(); }
      if(pasMin==null){ const m=lines[i].match(/;Pas en minutes;([^;\n]*)/i); if(m){ const v=(m[1]||'').trim(); if(/^\d+$/.test(v)) pasMin=parseInt(v,10); }}
    }
  }
...
function transformData(parsed, forced){
  ...
  if(iHorodate>=0 && iValeur>=0){
    ...
    // DEFAULT requested by user: if unit missing, assume 'W'
    ...
    diag.format='Horodate;Valeur'; diag.unit=unit; diag.stepMin=Math.round(stepMin); diag.count=data.length; diag.first=pts[0].t; diag.last=pts[pts.length-1].t;
    return {data, diag};
  }
...
function applyOverrides(kind){ const parsed = PARSED[kind]; if(!parsed) return; const forced={ unit: (OV[kind].unit||''), step: (OV[kind].step||null) }; try{ const {data,diag}=transformData(parsed, forced); setData(kind, data, diag); updateDiag(); recompute(true); } catch(err){ console.error(err); /* keep previous valid data */ } }
...
// Globals to preserve year on recompute
let LAST_SELECTED_YEAR = '';

// Core recompute; preserveYear -> keep current year selection
function recompute(preserveYear=false){ if(!DATA_IMPORT){ $('#results').classList.add('hidden'); $('#no-data').classList.remove('hidden'); return; }
  if(preserveYear){ LAST_SELECTED_YEAR = $('#year_select')?.value || ''; }
  const p=collectParams(); const imp=aggImportHPHC(DATA_IMPORT); const exp=aggSimple(DATA_EXPORT||[]); const pv=aggSimple(DATA_PV||[]); const SUM_HP=imp.sumHP, SUM_HC=imp.sumHC; const TOT_IMPORT=SUM_HP+SUM_HC, TOT_EXPORT=exp.total, TOT_PV=pv.total; const PV_AUTO=Math.max(0, TOT_PV - TOT_EXPORT); const LOAD=Math.max(0, TOT_PV + TOT_IMPORT - TOT_EXPORT); const pv_share=LOAD>0? PV_AUTO/LOAD : 0; $('#tot_import').textContent=fmtNum(TOT_IMPORT,3)+' kWh'; $('#tot_pv').textContent=(TOT_PV>0?fmtNum(TOT_PV,3)+' kWh':'—'); $('#tot_export').textContent=(TOT_EXPORT>0?fmtNum(TOT_EXPORT,3)+' kWh':'—'); $('#tot_pv_auto').textContent=(PV_AUTO>0?fmtNum(PV_AUTO,3)+' kWh':'—'); $('#tot_load').textContent=(LOAD>0?fmtNum(LOAD,3)+' kWh':'—'); $('#pv_share').textContent=(LOAD>0?fmtPct(pv_share):'—'); renderPie($('#pie'), SUM_HC, SUM_HP); renderPieLegend($('#pie_legend'), SUM_HC, SUM_HP);
  // Abonnements (global)
  ...
  // Build selector
  const impMap=imp.map, expMap=exp.map||new Map(), pvMap=pv.map||new Map(); const allKeys=[...new Set([ ...impMap.keys(), ...expMap.keys(), ...pvMap.keys() ])].sort(); const years=[...new Set(allKeys.map(k=>parseInt(k.split('-')[0],10)))].sort((a,b)=>a-b); const sel=$('#year_select'); sel.innerHTML=''; const oAll=document.createElement('option'); oAll.value=''; oAll.textContent='Toutes'; sel.appendChild(oAll); years.forEach(y=>{const o=document.createElement('option'); o.value=String(y); o.textContent=String(y); sel.appendChild(o);}); if(preserveYear){ sel.value = LAST_SELECTED_YEAR; }
  sel.onchange=()=>refreshMonthly(impMap,expMap,pvMap);
  refreshMonthly(impMap,expMap,pvMap); $('#results').classList.remove('hidden'); $('#no-data').classList.add('hidden'); $('#btn-export').onclick=()=>exportCSV(DATA_IMPORT, DATA_EXPORT, DATA_PV); }

// Year-filtered monthly view that ALSO updates KPIs & economies
function refreshMonthly(impMap,expMap,pvMap){ const yearSel=$('#year_select').value; LAST_SELECTED_YEAR = yearSel; const yr=yearSel?parseInt(yearSel,10):null; const keys=[...new Set([ ...impMap.keys(), ...expMap.keys(), ...pvMap.keys() ])].sort(); const rows=[]; keys.forEach(k=>{ const y=parseInt(k.split('-')[0],10); if(yr&&y!==yr) return; const hp=(impMap.get(k)?.hp)||0, hc=(impMap.get(k)?.hc)||0, pv=pvMap.get(k)||0, ex=expMap.get(k)||0; const pv_auto=Math.max(0, pv-ex); const totalLoad=Math.max(0, pv + hp + hc - ex); rows.push({key:k,label:label(k),hp,hc,pv_auto,totalLoad,pv,exp:ex}); }); drawStackedBars($('#monthly_chart'), rows); renderStackedLegend($('#bar_legend'));
  // Update KPIs & economies for filtered period
  ...
  // Abonnements sur période filtrée : mois = nb de lignes (mois) visibles
  const monthsCount = rows.length; const cout_base = totImport * p.prix_base; const cout_hphc = sumHC * p.prix_hc + sumHP * p.prix_hp; const tot_base_abos = cout_base + (p.abo_base||0)*monthsCount; const tot_hphc_abos = cout_hphc + (p.abo_hphc||0)*monthsCount; $('#tot_base_abos').textContent = fmtEuro(tot_base_abos); $('#tot_hphc_abos').textContent = fmtEuro(tot_hphc_abos); $('#eco_abos').textContent = fmtEuro(tot_base_abos - tot_hphc_abos);
  // Export monthly (filtered)
