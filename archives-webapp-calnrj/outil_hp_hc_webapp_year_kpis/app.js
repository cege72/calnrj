// App: CSV preview, overrides (unit/step), PV+Export, pie with %, and KPIs/economies updated by year filter
...
async function handleFile(inputSel, kind){ const el=$(inputSel); if(!el.files?.length){ setData(kind,null,null); PARSED[kind]=null; updateDiag(); recompute(); return; } const text=await el.files[0].text(); const parsed=parseCSVWithMeta(text); PARSED[kind]=parsed; renderPreview('#preview-'+kind, parsed); const forced={ unit: (OV[kind].unit||''), step: (OV[kind].step||null) }; const {data,diag}=transformData(parsed, forced); setData(kind, data, diag); updateDiag(); recompute(); }

function applyOverrides(kind){ const parsed = PARSED[kind]; if(!parsed) return; const forced={ unit: (OV[kind].unit||''), step: (OV[kind].step||null) }; try{ const {data,diag}=transformData(parsed, forced); setData(kind, data, diag); updateDiag(); recompute(); } catch(err){ console.error(err); /* keep previous valid data */ } }
...
function recompute(){ if(!DATA_IMPORT){ $('#results').classList.add('hidden'); $('#no-data').classList.remove('hidden'); return; } const p=collectParams(); const imp=aggImportHPHC(DATA_IMPORT); const exp=aggSimple(DATA_EXPORT||[]); const pv=aggSimple(DATA_PV||[]); const SUM_HP=imp.sumHP, SUM_HC=imp.sumHC; const TOT_IMPORT=SUM_HP+SUM_HC, TOT_EXPORT=exp.total, TOT_PV=pv.total; const PV_AUTO=Math.max(0, TOT_PV - TOT_EXPORT); const LOAD=Math.max(0, TOT_PV + TOT_IMPORT - TOT_EXPORT); const pv_share=LOAD>0? PV_AUTO/LOAD : 0; $('#tot_import').textContent=fmtNum(TOT_IMPORT,3)+' kWh'; $('#tot_pv').textContent=(TOT_PV>0?fmtNum(TOT_PV,3)+' kWh':'—'); $('#tot_export').textContent=(TOT_EXPORT>0?fmtNum(TOT_EXPORT,3)+' kWh':'—'); $('#tot_pv_auto').textContent=(PV_AUTO>0?fmtNum(PV_AUTO,3)+' kWh':'—'); $('#tot_load').textContent=(LOAD>0?fmtNum(LOAD,3)+' kWh':'—'); $('#pv_share').textContent=(LOAD>0?fmtPct(pv_share):'—'); renderPie($('#pie'), SUM_HC, SUM_HP); renderPieLegend($('#pie_legend'), SUM_HC, SUM_HP); // abos (global)
  ...
  // Build year selector and draw monthly
